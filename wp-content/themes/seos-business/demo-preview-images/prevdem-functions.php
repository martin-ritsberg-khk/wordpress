...
function seos_business_get_prevdem_img_src( $i = 0 ) {
	...
	if ( ! file_exists( $path . $img_name ) ) {
		...
		$i++;
		return seos_business_get_prevdem_img_src( $i );
	}
 ...
 */
function seos_business_the_post_thumbnail( $input ) {
	if ( empty( $input ) ) {
		$placeholder = seos_business_get_prevdem_img_src();
		return '<img style="width: 100%; height: auto;" src="' . esc_url( $placeholder ) . '" class="wp-post-image">';
	}
	return $input;
}

add_filter( 'post_thumbnail_html', 'seos_business_the_post_thumbnail' );
